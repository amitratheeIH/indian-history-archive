...
function buildFilters(data) {
  const fill = (el, vals, key, cnt) =>
    [...new Set(vals)].filter(Boolean).sort().forEach(v => el.appendChild(makeCheckbox(key,v,v,cnt(v))));
  ...
  fill($('langList'), langs, 'language', v=>data.filter(d=>[d.language,...(d.alternate_urls||[]).map(a=>a.language)].includes(v)).length);

  const fmts = data.flatMap(d=>[d.file_format,...(d.alternate_urls||[]).map(a=>a.format)]);
  fill($('formatList'), fmts, 'file_format', v=>data.filter(d=>[d.file_format,...(d.alternate_urls||[]).map(a=>a.format)].includes(v)).length);
  ...
  ['fl-sub','fl-dynasty','fl-region','fl-srctype','fl-lang','fl-format','fl-tags'].forEach(id => initCollapsible(id, false));
...
function getSet(key) {
  return ({type:AF.types,category:AF.categories,subcategory:AF.subcategories,
    era:AF.eras,dynasty:AF.dynasties,region:AF.regions,source_type:AF.sourceTypes,
    language:AF.languages,file_format:AF.formats,tag:AF.tags})[key];
}
...
function initCollapsible(id, open) {
  ...
  head.addEventListener('click', () => { body.classList.toggle('collapsed'); arrow?.classList.toggle('open'); });
}
  ...
  return data.filter(item => {
    if (q) {
      ...
      switch(searchScope) {
        ...
        default: hay=[item.title,item.description,
...
function showCategoryView() {
  flatView.style.display='none'; categoryView.style.display='';
  categoryView.innerHTML=''; backToBrowse.style.display='none';
...
async function showFlatView() {
  ...
  categoryView.style.display='none'; flatView.style.display='';
  ...
  if (!pageItems.length) {
    ...
    flatGrid.querySelectorAll('.empty-cat-btn').forEach(btn =>
      btn.addEventListener('click', () => {
        clearAll(false); AF.categories.add(btn.dataset.cat);
    ...
    flatGrid.innerHTML = `
      <table class="flat-table">
        ...
        <tbody>
          ${pageItems.map(item => tableRowHTML(item)).join('')}
        </tbody>
      </table>`;
  } else {
    flatGrid.className = 'flat-grid';
...
function cardHTML(item, delay=0, showBreadcrumb=false) {
  ...
  /* Breadcrumb — max 3 subcategories */
  ...
  /* Alternate URLs — compact single line pills */
    ...
    <div class="card-alt-urls">
      ${item.alternate_urls.map(a=>`
        <a class="card-alt-link" href="${a.url}" target="_blank" rel="noopener">
          <span class="alt-lang">${a.language}</span><span class="alt-label">${a.label}</span>
        </a>`).join('')}
    </div>` : '';
...
function tableRowHTML(item) {
  ...
  const alts   = (item.alternate_urls||[]).length
    ? `<span class="table-alts" title="${item.alternate_urls.map(a=>a.language).join(', ')}">+${item.alternate_urls.length} lang</span>`
    : '';
  ...
  return `
    <tr class="table-row${item.featured?' table-row-featured':''}">
      ...
      <td class="td-cat">
        ...
        <span class="td-cat-text">${cats}</span>
      </td>
...
function bindEvents() {
  ...
  viewToggleCard.addEventListener('click', () => {
    setViewToggle('card'); if (isFiltered()) showFlatView(); pushURL();
  });
  viewToggleTable.addEventListener('click', () => {
    setViewToggle('table'); if (isFiltered()) showFlatView(); else showCategoryView(); pushURL();
  });
